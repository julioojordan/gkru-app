...
const KeluargaDetail = () => {
  ...
  const [anggotaTable, setAnggotaTable] = useState(
    location.state?.data ? location.state.data.Anggota : null
  );
  ...
  const setLocalState = async () => {
    ...
    try {
      ...
      if (
        ...
        auth.ketuaLingkungan !== dataKeluarga.Lingkungan.Id
      ) {
        navigate(-1);
      }
      if (
        role.role === "ketuaWilayah" &&
        auth.ketuaWilayah !== dataKeluarga.Wilayah.Id
      ) {
        navigate(-1);
      }
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        const response =
          await services.HistoryService.getAllHistoryWithIdKeluarga(idKeluarga);
        if (response) {
  ...
  useEffect(() => {
    const fetchEditData = async () => {
      ...
      setLoadingEdit(true);
      try {
        const anggotaResponse =
          await services.AnggotaService.getAllAnggotaWithIdKeluarga(idKeluarga);
  ...
  const handleKepalaChange = (selectedOption) => {
    const selectedAnggota = anggota.find(
      (anggota) => anggota.Id === selectedOption.value
    );
    setFormData({
      ...formData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        handleKepalaChange(
          anggotaOptions.find(
            (option) => option.value === initialFormData.KepalaKeluarga
          )
        );
        handleLingkunganChange(
      ...
      <CCardBody>
        ...
        <div style={{ overflowX: "auto" }}>
          <CTable striped hover className="mt-4 shadow-sm">
            ...
            <CTableBody>
              {anggotaTable &&
              Array.isArray(anggotaTable) &&
              anggotaTable.length > 0 ? (
